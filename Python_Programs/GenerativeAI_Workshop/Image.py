...
def download_image(query, num_results=1):
    try:
        ...
        for result in search_results:
            ...
            if response.status_code == 200:
                ...
                if response.headers['content-type'].split('/')[0] == 'image':
                    ...
                    # Verify if the image is opened successfully
                    img.verify()
...
def main():
    ...
    if input_text:
        ...
        # Display the image along with other details
        image = download_image(input_text)
        if image:
            st.image(image, caption='Google Image Result', use_column_width=True)
        else:
            st.warning("No image found.")
